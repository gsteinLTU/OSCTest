...
namespace OscCore
{
    public class OscClient
    {
        ...
        public readonly OscWriter m_Writer;
        ...
        public void SendCustomFormat(string address, string typeTags, IEnumerable<object> elements)
        {
            m_Writer.Reset();
            m_Writer.Write(address);
            ...
            foreach (var element in elements)
            {
                switch (element)
                {
                    ...
                    case Color c2:
                        ...
                        break;
                    case byte[] bytes:
                        m_Writer.Write(bytes, bytes.Length, 0);
                        break;
                    case MidiMessage midiMessage:
                        m_Writer.Write(midiMessage);
                        break;
                }
            }
            m_Socket.Send(m_Writer.Buffer, m_Writer.Length, SocketFlags.None);
